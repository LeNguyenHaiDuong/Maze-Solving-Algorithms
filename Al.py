...
def Output_list(now, m, l):
    if now == m.start_node:
        return l
    td = now.pre_node[len(now.pre_node) - 1]
    l.append(td)
    Output_list(m.matrix[td[0]][td[1]], m, l)


def Update_Algo(m, explore, close):
    UCS(m.end_node, explore, close)
    l = []
    cl = []
    Output_list(m.end_node, m, l)
    for i in l:
        print(i)
        m.matrix[i[0]][i[1]]
        for j in (m.matrix[i[0]][i[1]]).neighbor_node:
            if j.self_node not in l and j.self_cost < -1 and j not in cl:
                m.end_node.total_cost += j.self_cost + 1
                m.matrix[i[0]][i[1]].pre_node.append(j.self_node)
                j.pre_node.append(m.matrix[i[0]][i[1]].self_node)
                cl.append(j)


def main():
    # - In ra ket qua
    m.read_file('input/level_2/input3.txt')
    # m.print_matrix()
